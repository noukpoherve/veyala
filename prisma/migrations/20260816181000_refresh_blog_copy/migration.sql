...
@@$seed$,
  "readingTimeMin" = 8,
  "updatedAt" = CURRENT_TIMESTAMP
WHERE "slug" = $seed$adapter-cv-offre-emploi$seed$;

UPDATE "BlogPost" SET
  "title" = $seed$Lettre de motivation : structure efficace qui convertit en 2026$seed$,
  "description" = $seed$Structure d'une lettre de motivation percutante : accroche, preuves, motivation entreprise, conclusion CTA. Exemples et erreurs à éviter pour décrocher un entretien.$seed$,
  "excerpt" = $seed$Une lettre utile n'est pas un résumé du CV. Voici la structure en 4 blocs qui fonctionne encore auprès des recruteurs.$seed$,
  "category" = $seed$LETTRE$seed$::"BlogCategory",
  "tags" = ARRAY[$seed$lettre de motivation$seed$, $seed$candidature$seed$, $seed$entretien$seed$]::TEXT[],
  "keywords" = ARRAY[$seed$lettre de motivation$seed$, $seed$structure lettre motivation$seed$, $seed$exemple lettre motivation$seed$, $seed$écrire lettre motivation$seed$, $seed$lettre motivation emploi$seed$, $seed$candidature$seed$, $seed$décrocher entretien$seed$, $seed$lettre motivation efficace$seed$]::TEXT[],
  "focusKeyword" = $seed$lettre de motivation$seed$,
  "featured" = TRUE,
  "accent" = $seed$#0284C7$seed$,
  "authorName" = $seed$Équipe Veyala$seed$,
  "authorRole" = $seed$Experts CV, ATS & candidature$seed$,
  "body" = $seed$[{"type":"p","text":"La lettre de motivation reste demandée pour de nombreuses candidatures en France (entreprises, administrations, écoles). Son rôle : montrer pourquoi vous et pourquoi eux, pas répéter le CV ligne à ligne."},{"type":"h2","text":"La structure en 4 blocs"},{"type":"h3","text":"1. Accroche (pourquoi ce poste, maintenant)"},{"type":"p","text":"Ouvrez sur le besoin de l'entreprise ou une réalisation liée au poste. Évitez « Je me permets de vous contacter… »."},{"type":"h3","text":"2. Preuves (2–3 arguments issus de votre parcours)"},{"type":"p","text":"Sélectionnez des expériences qui répondent aux missions de l'offre. Chiffres, responsabilités, impact."},{"type":"h3","text":"3. Motivation entreprise (pourquoi eux)"},{"type":"p","text":"Citez un projet, une valeur, un marché ou une actualité concrète. Montrez que vous avez lu l'annonce et l'entreprise."},{"type":"h3","text":"4. Conclusion + appel à l'action"},{"type":"p","text":"Proposez un échange. Restez court, confiant, sans formules creuses."},{"type":"h2","text":"Longueur et ton"},{"type":"ul","items":["Ideal : 250–400 mots (une page max)","Ton professionnel, actif, précis","Même vocabulaire métier que l'offre (sans copier-coller)","Cohérence totale avec le CV joint"]},{"type":"h2","text":"Erreurs qui tuent une lettre"},{"type":"ul","items":["Lettre générique recyclée pour 20 entreprises","Orthographe et prénom du recruteur approximatifs","Focus sur ce que vous voulez gagner, pas sur ce que vous apportez","Trop long, trop vague, trop humble ou trop arrogant"]},{"type":"cta","text":"Générez une lettre alignée sur votre CV et sur l'offre en quelques secondes.","href":"/register","label":"Créer ma lettre de motivation"}]$seed$::jsonb,
  "faq" = $seed$[{"question":"La lettre de motivation est-elle encore utile ?","answer":"Oui, surtout en France et pour les postes en tension qualitative (cadres, stages sélectifs, écoles). Elle différencie deux CV techniques proches."},{"question":"Faut-il une lettre si l'ATS ne la lit pas ?","answer":"Souvent le recruteur la lit après le filtre CV. Une lettre ciblée reste un avantage dès que votre dossier passe l'ATS."}]$seed$::jsonb,
  "bodyMarkdown" = $seed$La lettre de motivation reste demandée pour de nombreuses candidatures en France (entreprises, administrations, écoles). Son rôle : montrer pourquoi vous et pourquoi eux, pas répéter le CV ligne à ligne.
...
UPDATE "BlogPost" SET
  ...
  "bodyMarkdown" = $seed$Les générateurs de CV par IA explosent parce que le vrai goulot d'étranglement n'est plus « avoir un CV », c'est « avoir le bon CV pour cette offre ». Bien utilisée, l'IA devient un accélérateur de matching.
...
@@$seed$,
  "readingTimeMin" = 8,
  "updatedAt" = CURRENT_TIMESTAMP
WHERE "slug" = $seed$cv-ia-avantages-limites-bonnes-pratiques$seed$;

UPDATE "BlogPost" SET
  "title" = $seed$CV étudiant : stage, alternance, Parcoursup et Campus France$seed$,
  "description" = $seed$Réussir son CV étudiant pour stage, alternance, Parcoursup ou Campus France : projets, soft skills, formations et lettre de motivation adaptée aux dossiers scolaires.$seed$,
  "excerpt" = $seed$Peu d'expérience pro ? Votre CV étudiant peut quand même convaincre si vous valorisez projets, associations et résultats académiques.$seed$,
  "category" = $seed$ETUDES$seed$::"BlogCategory",
  "tags" = ARRAY[$seed$étudiant$seed$, $seed$stage$seed$, $seed$Campus France$seed$, $seed$Parcoursup$seed$]::TEXT[],
  "keywords" = ARRAY[$seed$CV étudiant$seed$, $seed$CV stage$seed$, $seed$CV alternance$seed$, $seed$Campus France CV$seed$, $seed$Parcoursup$seed$, $seed$lettre motivation étudiant$seed$, $seed$candidature stage$seed$, $seed$CV première expérience$seed$, $seed$dossier candidature formation$seed$]::TEXT[],
  "focusKeyword" = $seed$CV étudiant$seed$,
  "featured" = FALSE,
  "accent" = $seed$#0EA5E9$seed$,
  "authorName" = $seed$Équipe Veyala$seed$,
  "authorRole" = $seed$Experts CV, ATS & candidature$seed$,
  "body" = $seed$[{"type":"p","text":"Pour un stage, une alternance ou un dossier Campus France / formation sélective, le jury ne cherche pas 10 ans d'expérience : il cherche du potentiel, de la cohérence et des preuves d'engagement."},{"type":"h2","text":"Ce qui compte sur un CV étudiant"},{"type":"ul","items":["Projets académiques avec livrable et rôle clair","Associations, jobs étudiants, bénévolat (responsabilités)","Compétences outils (Excel, Python, Figma, langues…)","Résultats : classement, prix, KPIs d'un projet, volume géré"]},{"type":"h2","text":"Stage & alternance : cibler l'entreprise"},{"type":"p","text":"Même logique que pour un emploi : adaptez le CV à l'offre. Mettez en avant les cours et projets proches des missions. Ajoutez une lettre courte qui explique votre motivation pour ce métier / cette boîte."},{"type":"h2","text":"Campus France & dossiers formation"},{"type":"p","text":"Les dossiers études valorisent la cohérence du projet pédagogique. Votre CV et votre lettre doivent raconter la même histoire : pourquoi cette formation, pourquoi maintenant, quelles preuves de capacité de travail."},{"type":"callout","title":"Veyala Étudiants","text":"Collez une fiche de formation ou une offre de stage : Veyala génère un CV et une lettre adaptés au contexte études ou emploi."},{"type":"cta","text":"Préparez un dossier clair pour stage, alternance ou formation.","href":"/register","label":"Créer mon CV étudiant"}]$seed$::jsonb,
  "faq" = $seed$[]$seed$::jsonb,
  "bodyMarkdown" = $seed$Pour un stage, une alternance ou un dossier Campus France / formation sélective, le jury ne cherche pas 10 ans d'expérience : il cherche du potentiel, de la cohérence et des preuves d'engagement.
...
@@$seed$,
  "readingTimeMin" = 6,
  "updatedAt" = CURRENT_TIMESTAMP
WHERE "slug" = $seed$mots-cles-cv-par-metier$seed$;

UPDATE "BlogPost" SET
  "title" = $seed$Stratégie job hunting : candidater mieux (pas seulement plus)$seed$,
  "description" = $seed$Stratégie de recherche d'emploi efficace : ciblage des offres, CV ATS par candidature, suivi, networking LinkedIn et préparation aux entretiens.$seed$,
  "excerpt" = $seed$Postuler en masse fatigue. Une stratégie job hunting mise sur la qualité du matching CV–offre et un suivi rigoureux.$seed$,
  "category" = $seed$EMPLOI$seed$::"BlogCategory",
  "tags" = ARRAY[$seed$emploi$seed$, $seed$job hunting$seed$, $seed$recrutement$seed$, $seed$LinkedIn$seed$]::TEXT[],
  "keywords" = ARRAY[$seed$job hunting$seed$, $seed$recherche emploi$seed$, $seed$stratégie candidature$seed$, $seed$trouver un job$seed$, $seed$candidature efficace$seed$, $seed$LinkedIn emploi$seed$, $seed$entretien embauche$seed$, $seed$marché de l'emploi$seed$, $seed$CV et lettre$seed$]::TEXT[],
  "focusKeyword" = $seed$job hunting$seed$,
  "featured" = FALSE,
  "accent" = $seed$#1E40AF$seed$,
  "authorName" = $seed$Équipe Veyala$seed$,
  "authorRole" = $seed$Experts CV, ATS & candidature$seed$,
  "body" = $seed$[{"type":"p","text":"Le volume de candidatures compte, mais le taux de réponse dépend surtout de la pertinence. Voici un cadre simple pour chercher un emploi sans s'épuiser."},{"type":"h2","text":"1. Définir 2–3 cibles nettes"},{"type":"p","text":"Métier, niveau, type d'entreprise, remote/hybride. Trop large = CV dilué et message confus."},{"type":"h2","text":"2. Qualifier chaque offre avant de postuler"},{"type":"ul","items":["Match ≥ 60–70 % des must-have","Salaire / localisation acceptables","Annonce claire (pas de red flags extrêmes)"]},{"type":"h2","text":"3. Un dossier par candidature"},{"type":"p","text":"CV adapté + lettre courte + profil LinkedIn cohérent. Stockez version et date dans un tableau de suivi."},{"type":"h2","text":"4. Activer le réseau (sans spam)"},{"type":"p","text":"Message LinkedIn personnalisé, demande d'avis sur le rôle, pas de « je cherche un job » générique."},{"type":"h2","text":"5. Mesurer et itérer"},{"type":"p","text":"Si 20 candidatures ciblées = 0 réponse, le problème est le matching ou le positionnement, pas seulement « le marché »."},{"type":"cta","text":"Accélérez chaque candidature avec un CV + lettre générés à partir de l'offre.","href":"/register","label":"Candidater plus vite avec Veyala"}]$seed$::jsonb,
  "faq" = $seed$[]$seed$::jsonb,
  "bodyMarkdown" = $seed$Le volume de candidatures compte, mais le taux de réponse dépend surtout de la pertinence. Voici un cadre simple pour chercher un emploi sans s'épuiser.
...
@@$seed$,
  "readingTimeMin" = 7,
  "updatedAt" = CURRENT_TIMESTAMP
WHERE "slug" = $seed$job-hunting-strategie-candidatures$seed$;

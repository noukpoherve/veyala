...
/**
 * Shared headless Chromium for PDF rendering and last-resort job-page fetches.
 * The browser instance is reused across requests in local dev.
 */
...
async function launchBrowser(): Promise<Browser> {
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        "Aucun navigateur Chromium disponible. " +
...
/** Tears down the shared browser so the next call launches a fresh one. */
...
async function runWithLiveBrowser<T>(fn: (browser: Browser) => Promise<T>): Promise<T> {
...
/**
 * Runs `fn` against a live Chromium. Retries once if the shared process died
 * mid-call. Serverless hosts close the browser afterwards to cap memory.
 */
export async function withBrowser<T>(fn: (browser: Browser) => Promise<T>): Promise<T> {
  try {
    try {
      return await runWithLiveBrowser(fn);
    } catch (error) {
      // "Target page, context or browser has been closed" means the shared
      // Chromium crashed. Retry once on a fresh browser: transient memory
      // pressure usually clears after the crashed process is reaped.
    ...
    // Chromium warm lets memory accumulate across CV/cover-letter renders and
    // job fetches and OOM-kills the process. Close it after each use so peak
    // usage is ever only a single session. Local dev keeps the browser warm.

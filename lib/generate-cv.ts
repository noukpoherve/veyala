...
/**
 * Full generation pipeline: job text → credit debit → LLM tailoring →
 * DOCX + PDF rendering → storage → GeneratedCV record.
...
export async function generateCV(params: GenerateParams) {
  ...
  try {
    ...
    const html = renderCVHtml(cv, definition);
    const [pdf, docx] = await Promise.all([htmlToPdf(html), renderCVDocx(cv, definition)]);

    const base = `CV_${slugify(cv.identity.fullName)}_${slugify(detectedTitle)}`;
    const [pdfFile, docxFile] = await Promise.all([
      saveFile(pdf, { dir: `exports/${userId}`, filename: `${base}.pdf`, contentType: "application/pdf" }),
      saveFile(docx, {
        dir: `exports/${userId}`,
        filename: `${base}.docx`,

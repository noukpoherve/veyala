...
export async function testProvider(formData: FormData): Promise<{ ok: boolean; message: string }> {
  ...
  try {
    ...
    if (providerId) {
      ...
      if (!row?.apiKeyEnc) return { ok: false, message: m.api.settings.providerNotFound };
      config = {
    ...
    const msg =
      e instanceof LLMError || e instanceof Error ? e.message : m.api.settings.providerTestFailed;

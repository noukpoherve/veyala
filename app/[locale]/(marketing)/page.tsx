...
const STEPS = [
  { icon: Upload, iconBg: "from-blue-500 to-blue-600", number: "01" },
  { icon: FileText, iconBg: "from-sky-400 to-blue-500", number: "02" },
  { icon: Download, iconBg: "from-blue-700 to-indigo-800", number: "03" },
] as const;

const FEATURES = [
  { icon: ScanSearch, chip: "bg-blue-50 text-blue-600" },
  { icon: Layers, chip: "bg-blue-50 text-blue-600" },
  { icon: FileDown, chip: "bg-blue-50 text-blue-600" },
  { icon: Cpu, chip: "bg-violet-50 text-violet-600" },
  { icon: ShieldCheck, chip: "bg-emerald-50 text-emerald-600" },
  { icon: PenLine, chip: "bg-amber-50 text-amber-600" },
] as const;
...
function HeroVisual() {
  const m = getMessages(getLocale()).marketing;
      ...
      <div className="absolute left-0 top-24 hidden -rotate-6 md:block">
        <div className="animate-float-b w-48 rounded-2xl bg-white/90 p-4 text-left shadow-xl shadow-blue-900/10 ring-1 ring-slate-900/5 backdrop-blur">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold text-slate-800">{m.heroAtsScore}</span>
          </div>
      ...
      <div className="absolute right-0 top-16 hidden rotate-6 md:block">
        <div className="animate-float-b w-44 rounded-2xl bg-white/90 p-4 text-left shadow-xl shadow-blue-900/10 ring-1 ring-slate-900/5 backdrop-blur">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold text-slate-800">{m.heroAiWorking}</span>
          </div>
          ...
          <div className="mt-3 flex items-center gap-1.5">
            ...
            <span className="text-[10px] text-slate-400">{m.heroGenerating}</span>
          </div>
        </div>
      </div>
              ...
              heading={
                <>
                  <span className="inline-flex items-center gap-2 rounded-full bg-blue-50 px-4 py-1.5 text-xs font-bold uppercase tracking-wide text-blue-700">
                    ...
                    {m.marketing.templatesEyebrow}
                  </span>
                  <h2 className="mt-6 font-display text-4xl font-extrabold tracking-tight text-slate-900 md:text-5xl">
                    {m.marketing.templatesTitleBefore}
                    <span className="text-blue-600">{m.marketing.templatesTitleHighlight}</span>
                  </h2>
                  <p className="mt-4 text-lg text-slate-500">{m.marketing.templatesSubtitle}</p>
                </>
              }
            />
          </Reveal>
        ...
        <div className="mx-auto max-w-6xl px-6 py-24">
          ...
          <div className="mx-auto mt-16 grid max-w-5xl items-start gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {packs.map((pack, index) => {
              const featured = index === FEATURED_PLAN_INDEX;

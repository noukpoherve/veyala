...
export default async function BlogPostPage({ params }: PageProps) {
  ...
  const schemas = [blogPostingJsonLd(post), blogBreadcrumbJsonLd(post), blogFaqJsonLd(post)].filter(
    Boolean
  );
          ...
          <div className="relative mx-auto max-w-3xl px-6 pb-12 pt-10 md:pb-14 md:pt-14">
            <nav aria-label="Fil d'Ariane" className="mb-8">
              <ol className="flex flex-wrap items-center gap-2 text-sm text-slate-500">
                ...
                <li aria-hidden>/</li>
                <li className="truncate text-slate-700">{CATEGORY_LABELS[post.category]}</li>
              </ol>
            </nav>
            ...
            <div className="mt-8 flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-slate-500">
              ...
              <time dateTime={post.publishedAt}>Publié le {formatPostDate(post.publishedAt)}</time>
        ...
        <div className="mx-auto max-w-3xl px-6 py-12 md:py-14">
          ...
          {post.faq?.length ? (
            <section className="mt-14 border-t border-slate-100 pt-10">
              <h2 className="font-display text-2xl font-bold tracking-tight text-slate-900">FAQ</h2>
